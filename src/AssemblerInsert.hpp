...
namespace AssemblerInsert {
	...
	public:
		...
		std::string get() {
			...
			ret += outset();
			if (!libraries.empty() || !imports.empty()) ret += section_IDATA;
			for (std::string inc : includes) ret += "include '" + System::Text::replace(inc, "\n", "") + "'" + "\n";
			if (!libraries.empty()) {
				ret += "library ";
	...
	public:
		...
		// subtract -> [esp-n] || [esp+n]
		std::string	PushInStack(int size, bool subtract = true) {
			StackSize += (size * 2);
			StackSizes.push(StackSize / 2);
			if (subtract) return "[" + Stack + "-" + std::to_string(StackSizes.top()) + "]";
	...
	public:
		...
		static std::string invoke(std::string name, std::multimap<int, std::string> args, FinalCode *code) {
			std::string ret;
			ret += "\tcinvoke " + name;
	...
	void EndEntryPoint(FinalCode *code) {
		*code += "\tmov " + StackFrame + ", " + Stack + "\n";
		*code += "\tpop " + Stack + "\n";
		*code += "\tcall _StopProgram\n";
		*code += "\tret\n";
		*code += "\n";
	}
	void _StopProgram(FinalCode *code) {
		*code += Library::use("ExitProcess", "kernel32", "INCLUDE\\win32a.inc");
		*code += {
			"_StopProgram:\n"
