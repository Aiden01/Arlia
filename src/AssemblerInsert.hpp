...
namespace AssemblerInsert {
	...
	public:
		...
		std::string	PushInStack(int size, bool sub = true) {
			StackSize += (size * 2);
	...
	std::string SetArray(std::string name, int dimension, int bits, std::vector<std::string> values = { "" }, size_t size = 0) {
		if (values.size() > 0 && size == 0) size = values.size(); else size += 1;
		std::string ret;
		Register reg;
		std::string current = reg.next(bits);
		ret += "\tneg " + current + "\n";
		ret += "\tlea " + current + ", " + "[" + Stack + "-" + std::to_string(bits) + "+" + current + "]\n";
		for (size_t i = 0; i < size; ++i)
			ret += "\tmov " + OperatorIdentifier(bits) + " " + reg.PushInStack(bits, false) + ", " + values[i] + "\n";
		return ret;
	...
	std::string AccessItemArray(std::string name, int index) {
		return "...";
	}
