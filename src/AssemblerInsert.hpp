...
namespace AssemblerInsert {
	...
	private:
		struct GlobalVariable {
			std::string GVUN; // Global Variable Unique Name
			std::string bits;
			std::string value;
		};
		...
		const std::string section_DATA = "section '.data' data readable writeable ; data\n";
	...
	public:
		...
		void AppendGlobalVariable(int bits, std::string value = "?") {
			std::string bit;
		BitLoop:
			switch (bits) {
			case 0: bit = "db"; break;
		...
		std::string get() {
			...
			if (GVUN_n > 0) {
				ret += "\n" + section_DATA;
				for (GlobalVariable GV : GlobalVariableList) ret += "\t" + GV.GVUN + " " + GV.bits + " " + GV.value + "\n";
			}
			ret += "\n" + section_CODE + "\n";
			ret += RawCode;
			return ret;
	...
	public:
		...
		static std::string invoke(std::string name, std::multimap<int, std::string> args, AssemblerInsert::FinalCode *code) {
			std::string ret;
			ret += "\tcinvoke " + name;
			for (std::map<int, std::string>::iterator it = args.begin(); it != args.end(); ++it) {
				if (System::Text::IsString(it->second)) {
					typedef void (AssemblerInsert::FinalCode::*AppendGlobalVariable)(int, std::string);
					(code->AppendGlobalVariable)(it->first, it->second);
					ret += ", GVUN" + std::to_string(GVUN_n - 1);
				}
				else if (System::Text::IsChar(it->second)) ret += ", " + std::to_string((char)
					(System::Text::StringToChar(it->second.substr(1, it->second.size() - 2))));
				else ret += ", " + it->second;
			}
			return ret + "\n";
		}
	...
	public:
		...
		std::string MovToStack(size_t bits, std::string value) {
			std::string ret;
			if (System::Text::IsString(value)) {
				(code->AppendGlobalVariable)(sizeof(char), value);
				ret += "mov " + OperatorIdentifier(bits) + " " + reg.PushInStack(bits) + ", " + "GVUN" + std::to_string(GVUN_n - 1);
			}
			else if (System::Text::IsChar(value))
				ret += "mov " + OperatorIdentifier(bits) + " " + reg.PushInStack(bits) + ", " + std::to_string((int)System::Text::StringToChar(value));
			else ret += "mov " + OperatorIdentifier(bits) + " " + reg.PushInStack(bits) + ", " + value;
			return "\t" + ret + "\n";
		}
		std::string MovToReg(size_t bits, std::string value, std::string reg) {
			std::string ret;
			if (System::Text::IsString(value)) {
				(code->AppendGlobalVariable)(sizeof(char), value);
				ret += "mov " + OperatorIdentifier(bits) + " " + reg + ", " + "GVUN" + std::to_string(GVUN_n - 1);
			}
			else if (System::Text::IsChar(value))
				ret += "mov " + OperatorIdentifier(bits) + " " + reg + ", " + std::to_string((int)System::Text::StringToChar(value));
			else ret += "mov " + OperatorIdentifier(bits) + " " + reg + ", " + value;
			return "\t" + ret + "\n";
		}
		void RefreshStack() {
			reg.ResetStack();
	...
	std::string SetEntryPoint(bool *main) {
		std::string ret =
			"main:\n"
			"\tpush " + Stack + "\n"
	...
	std::string EndEntryPoint() {
		...
		ret += "\tpop " + Stack + "\n";
		ret += "\tcall _StopProgram\n";
		ret += "\tret\n";
		ret += "\n";
		return ret;
	}
	void _StopProgram(AssemblerInsert::FinalCode *code) {
		*code += AssemblerInsert::Library::use("ExitProcess", "kernel32", "INCLUDE\\win32a.inc");
		*code += {
			"_StopProgram:\n"
	...
	std::string SetArray(std::string identifier, int bits, size_t size = 0, std::vector<std::string> values = { "" }) {
		if (values.size() > 0 && size == 0) size = values.size(); else size += 1;
		std::string ret;
		std::string current = "eax";
		ret += "\tsub " + Stack + ", " + std::to_string(bits) + "\n";
		ret += "\tsub " + Stack + ", " + current + "\n";
		ret += "\tneg " + current + "\n";
		for (size_t i = 0; i < size; ++i) {
			ret += "\t" + identifier + "_" + std::to_string(i) + ":\n";
			// ret += heap->MovToReg(bits, values[i], current);
		}
		return ret;
	}
